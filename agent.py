...
class Three:
    ...
    def run_active_inference(self, epochs):
        ...
        A_I_b = A.copy()
        B_I_b = create_B_matrix()
        C_I_b = utils.onehot(observation.index(4), n_observation)
        D_I_b = utils.onehot(states.index(4), n_states)

        A_R_c = A.copy()
        B_R_c = create_B_matrix()
        C_R_c = utils.onehot(observation.index(9), n_observation)  # preference
        D_R_c = utils.onehot(states.index(7), n_states)  # priors over initial state

        A_S_c = A.copy()
        B_S_c = create_B_matrix()
        C_S_c = utils.onehot(observation.index(5), n_observation)
        D_S_c = utils.onehot(states.index(5), n_states)

        A_I_c = A.copy()
        B_I_c = create_B_matrix()
        C_I_c = utils.onehot(observation.index(4), n_observation)
        D_I_c = utils.onehot(states.index(4), n_states)
        ...
        for j in range(epochs):
            ...
            if j == 0:
                obs_idx_r_b, qs_curr_r_b, F_r_b = active_inference_with_planning(
                    A_R_b, B_R_b, C_R_b, D_R_b, 9, n_actions, self.env_r, policy_len=2, T=2)
                C_S_b = change_C(obs_idx_s_a)
                obs_idx_s_b, qs_curr_s_b, F_s_b = active_inference_with_planning(
                    A_S_b, B_S_b, C_S_b, D_S_b, 5, n_actions, self.env_s, policy_len=4, T=1)
                obs_idx_i_b, qs_curr_i_b, F_i_b = active_inference_with_planning(
                    A_I_b, B_I_b, C_I_b, D_I_b, 4, n_actions, self.env_i, policy_len=2, T=1)
            else:
                obs_idx_r_b, qs_curr_r_b, F_r_b = active_inference_with_planning(
                    A_R_b, B_R_b, C_R_b, D_R_b, obs_idx_r_b, n_actions, self.env_r, policy_len=2, T=2)
                obs_idx_s_b, qs_curr_s_b, F_s_b = active_inference_with_planning(
            ...
            if j == 0:
                obs_idx_r_c, qs_curr_r_c, F_r_c = active_inference_with_planning(
                    A_R_c, B_R_c, C_R_c, D_R_c, 3, n_actions, self.env_r, policy_len=2, T=2)
                C_S_b = change_C(obs_idx_s_a)
                obs_idx_s_c, qs_curr_s_c, F_s_c = active_inference_with_planning(
                    A_S_c, B_S_c, C_S_c, D_S_c, 6, n_actions, self.env_s, policy_len=4, T=1)
                obs_idx_i_c, qs_curr_i_c, F_i_c = active_inference_with_planning(
                    A_I_c, B_I_c, C_I_c, D_I_c, 4, n_actions, self.env_i, policy_len=2, T=1)
            else:
                obs_idx_r_c, qs_curr_r_c, F_r_c = active_inference_with_planning(
                    A_R_c, B_R_c, C_R_c, D_R_c, obs_idx_r_c, n_actions, self.env_r, policy_len=2, T=2)
                obs_idx_s_c, qs_curr_s_c, F_s_c = active_inference_with_planning(
                    A_S_c, B_S_c, C_S_c, D_S_c, obs_idx_s_c, n_actions, self.env_s, policy_len=4, T=1)
                C_S_c = change_C(obs_idx_s_b)
                obs_idx_i_c, qs_curr_i_c, F_i_c = active_inference_with_planning(
                    A_I_c, B_I_c, C_I_c, D_I_c, obs_idx_i_c, n_actions, self.env_i, policy_len=2, T=1)

            R_c = residual(C_R_c, qs_curr_r_c) + residual(C_S_c, qs_curr_s_c) + residual(C_I_c, qs_curr_i_c)
            D_R_c = D_update(D_R_c, F_r_c, R_c, 0.2)
            D_S_c = D_update(D_S_c, F_s_c, R_c, 3)
            D_I_c = D_update(D_I_c, F_i_c, R_c, 5)

            self.trajectory_c.append([obs_idx_r_c, obs_idx_s_c, obs_idx_i_c])

    def plot_trajectories(self):

        # Check if trajectory data exists and is not empty
        if hasattr(self, 'trajectory_a') and len(self.trajectory_a) > 0:
            trajectory_a = np.array(self.trajectory_a)
        else:
            print("Warning: No data for trajectory A")
            return

        if hasattr(self, 'trajectory_b') and len(self.trajectory_b) > 0:
            trajectory_b = np.array(self.trajectory_b)
        else:
            print("Warning: No data for trajectory B")
            return

        if hasattr(self, 'trajectory_c') and len(self.trajectory_c) > 0:
            trajectory_c = np.array(self.trajectory_c)
        else:
            print("Warning: No data for trajectory C")
            return
        ...
        ax.plot(trajectory_a[:, 0], trajectory_a[:, 1], trajectory_a[:, 2], c='red', linewidth=1, linestyle='-',
                label='Agent A')
        ax.plot(trajectory_b[:, 0], trajectory_b[:, 1], trajectory_b[:, 2], c='black', linewidth=1, linestyle='-',
                label='Agent B')
        ax.plot(trajectory_c[:, 0], trajectory_c[:, 1], trajectory_c[:, 2], c='green', linewidth=1, linestyle='-',
                label='Agent C')

        # Scatter plot for individual points
        ax.scatter(trajectory_a[:, 0], trajectory_a[:, 1], trajectory_a[:, 2], c='red', s=25, marker='o')
        ax.scatter(trajectory_b[:, 0], trajectory_b[:, 1], trajectory_b[:, 2], c='black', s=25, marker='o')
        ax.scatter(trajectory_c[:, 0], trajectory_c[:, 1], trajectory_c[:, 2], c='green', s=25, marker='o')
